...
;(async function() {
  try {
    ...
    const api = {}
    const resources = {}
    api.graphql = octokitGraphql.defaults({headers: {authorization: `token ${token}`}, baseUrl: _github_api_graphql || undefined})
    ...
    if (_notice_releases) {
      const {data: [{tag_name: tag}]} = await rest.repos.listReleases({owner: "embedx-io", repo: "metrics"})
